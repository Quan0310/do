    ...
    def build_targets(self, pred_boxes, ground_truth, height, width):
        ...
        for b in range(batch_size):
            ...
            for i, anno in enumerate(ground_truth[b]):
                gt[i, 0] = float((anno[0] + anno[2] / 2) / self.reduction)
                gt[i, 1] = float((anno[1] + anno[3] / 2) / self.reduction)
                gt[i, 2] = float(anno[2] / self.reduction)
                gt[i, 3] = float(anno[3] / self.reduction)
